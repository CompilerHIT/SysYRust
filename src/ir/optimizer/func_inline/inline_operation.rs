    ...
    bfs_inst_process(caller.get_head(), |inst| {
        if let InstKind::Call(name) = inst.get_kind() {
            if name == callee_name {
                ...
                inline_func_with_inst(inst, callee, callee_name, arg_list, pools);
            }
        }
    })
}
...
fn inline_func_with_inst(
    ...
    let (copyed_head_bb, mut copyed_end_bb) = copy_func(callee_name, callee, arg_list, pools);
    ...
    // 设置cfg路径
    if new_bb.get_tail_inst().is_jmp() {
        debug_assert_eq!(source_bb.get_next_bb().len(), 1);
        // 将source_bb的后继改为copyed_head_bb
        let next_bb = source_bb.get_next_bb()[0];
        source_bb.remove_next_bb(next_bb);
        source_bb.add_next_bb(copyed_head_bb);

        // 将new_bb的后继改为next_bb
        new_bb.add_next_bb(next_bb);
    } else {
        debug_assert_eq!(source_bb.get_next_bb().len(), 2);
        // 将source_bb的后继改为copyed_head_bb
        let first_next_bb = source_bb.get_next_bb()[0];
        let second_next_bb = source_bb.get_next_bb()[1];
        source_bb.remove_next_bb(first_next_bb);
        source_bb.remove_next_bb(second_next_bb);
        source_bb.add_next_bb(copyed_head_bb);

        // 将new_bb的后继改为next_bb
        new_bb.add_next_bb(first_next_bb);
        new_bb.add_next_bb(second_next_bb);
    }
