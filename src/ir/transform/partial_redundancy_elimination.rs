...
pub fn pre_congruence(congruence: &mut Congruence,head:ObjPtr<BasicBlock>,pools: &mut (&mut ObjPool<BasicBlock>, &mut ObjPool<Inst>)){
    ...
    for index_class in 0..congruence.vec_class.len(){
        loop{
            ...
            changed |= pre_group(&mut pre_context,head, &mut dominator_tree, &mut downstream_tree, congruence, index_class,pools);
...
pub fn pre_group(pre_context: &mut PreContext,head:ObjPtr<BasicBlock>,dominator_tree:&mut DominatorTree,downstream_tree:&mut DownStreamTree,congruence: &mut Congruence,index_class:usize,pools: &mut (&mut ObjPool<BasicBlock>, &mut ObjPool<Inst>)) ->bool{
    let mut changed = false;
    loop{
        let mut flag = false;
        ...
        for i in 0..congruence.vec_class[index_class].len(){
            for j in 0..congruence.vec_class[index_class].len(){
                if i==j{
                    ...
                    if downstream_tree.is_upstream(congruence.vec_class[index_class][j].get_parent_bb(), congruence.vec_class[index_class][i].get_parent_bb()){
                        ...
                        flag |=true;
                        changed |=true;
                        if insert_inst_in_pre(index_class,congruence,congruence.vec_class[index_class].clone(), pre_context, congruence.vec_class[index_class][i], &dominator_tree, pools.1, pools.0){
                            println!("计算新树");
...
pub fn insert_inst_in_pre(index:usize,congruence: &mut Congruence,gp: Vec<ObjPtr<Inst>>, pre_context: &mut PreContext,inst_old:ObjPtr<Inst>, dominator_tree: &DominatorTree, pool_inst: &mut ObjPool<Inst>, pool_block: &mut ObjPool<BasicBlock>)->bool{
    ...
    for i in 0..bb.get_up_bb().len(){
        if dominator_tree.is_dominate(&bb, &pres[index_temp]){//前继是自己所支配的，相应插的phi参数为phi本身
