...
impl Kit<'_> {
    ...
    pub fn get_var(&mut self, s: &str) -> Result<(ObjPtr<Inst>, Symbol), Error> {
        ...
        let sym_opt = self
            ...
            .get(s)
            .and_then(|vec_temp| vec_temp.last())
            .and_then(|(last_elm, layer)| {name_changed = last_elm.clone();layer_var = *layer;self.context_mut.symbol_table.get(last_elm)})
            .map(|x| x.clone());
    ...
    fn process(&mut self, input: Self::Message, kit_mut: &mut Kit) -> Result<Self::Ret, Error> {
        match self.btype {
            BType::Int => {
                for def in &mut self.const_def_vec {
                        if def.const_exp_vec.is_empty(){
                                    let (mut inst_ptr,mut val) = def.const_init_val.process(Type::ConstInt, kit_mut).unwrap();
                                if !kit_mut
                                    .context_mut
                                    .add_var(&def.ident, Type::ConstInt, false, Vec::new()){
                                ...
                                if kit_mut.context_mut.get_layer()<0{
                                    ...
                                    inst_ptr = kit_mut.pool_inst_mut.make_global_int_const(bond);//这里
                                }
                                kit_mut.context_mut.update_var_scope_now(&def.ident, inst_ptr);
                        }else{
                            todo!()
                        }                    
                }
            ...
            BType::Float => {
                for def in &mut self.const_def_vec {
                    if def.const_exp_vec.is_empty(){
                        let (mut inst_ptr,mut val) = def.const_init_val.process(Type::ConstFloat, kit_mut).unwrap();
                    if !kit_mut
                        .context_mut
                        .add_var(&def.ident, Type::ConstFloat, false, Vec::new()){
                            return Err(Error::MultipleDeclaration);
                        }
                        ...
                        if kit_mut.context_mut.get_layer()<0{
                            ...
                            inst_ptr = kit_mut.pool_inst_mut.make_global_float_const(bond);//这里
                        }
                    kit_mut.context_mut.update_var_scope_now(&def.ident, inst_ptr);
            }else{
                todo!()
    ...
    fn process(&mut self, input: Self::Message, kit_mut: &mut Kit) -> Result<Self::Ret, Error> {
        match self {
            ConstInitVal::ConstExp(constexp) =>{
                constexp.process(input, kit_mut)
            }
                ...
                for def in &mut self.var_def_vec {
                    match def {
                        VarDef::NonArrayInit((id, val)) => match val {
                            InitVal::Exp(exp) => {
                                let  (mut inst_ptr,mut val) = exp.process(Type::Int, kit_mut).unwrap();
                                if !kit_mut
                                    .context_mut
                                    ...
                                    if kit_mut.context_mut.get_layer()<0{
                                        match val {
                                            ExpValue::Int(i) =>{
                                                inst_ptr = kit_mut.pool_inst_mut.make_global_int(i);//这里
                                            }
                                            _=>{
                                                unreachable!()
                                            }
            ...
            BType::Float => {
                for def in &mut self.var_def_vec {
                    match def {
                        VarDef::NonArrayInit((id, val)) => match val {
                            InitVal::Exp(exp) => {
                                let (mut inst_ptr,val) = exp.process(Type::Float, kit_mut).unwrap();
                                if !kit_mut
                                    .context_mut
                                    ...
                                    if kit_mut.context_mut.get_layer()<0{
                                        match val {
                                            ExpValue::Float(f) =>{
                                                inst_ptr = kit_mut.pool_inst_mut.make_global_float(f);//这里
                                            }
                                            _=>{
                                                unreachable!()
                                            }
