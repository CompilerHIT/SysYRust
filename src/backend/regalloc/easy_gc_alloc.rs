...
impl Allocator {
    ...
    pub fn color(&mut self) -> bool {
        ...
        while !self.regs.is_empty() {
            ...
            if !reg.is_virtual()||self.spillings.contains(&reg.get_id())||self.colors.contains_key(&reg.get_id()) {
                self.regs.pop_front();
                continue;
            }
    ...
    pub fn color_one(&mut self,reg:Reg)->bool {
        ...
        Allocator::debug("color", [&reg,&Reg::new(color, reg.get_type())].into_iter().collect());
    ...
    pub fn decolor_one(&mut self,reg:Reg)->bool {
        ...
        Allocator::debug("decolor", [&reg,&Reg::new(color, reg.get_type())].into_iter().collect());
        if let Some(neighbors) = self.interference_graph.get(&reg){
            for neighbor in neighbors{
                ...
                if new_num==0 {
                    self.availables.get_mut(&neighbor).unwrap().release_reg(color);
                }
            }
        }
        true
    ...
    pub fn simplify_one(&mut self,target_reg:&Reg)->bool{    
        ...
        for neighbor in neighbors { 
            ...
            if !neighbor.is_virtual() {continue;}
            if self.spillings.contains(&neighbor.get_id()) {continue;}
            ...
            if self.availables.get(&neighbor).unwrap().num_available_regs(target_reg.get_type())<=1 {
                continue;
            }
            // TODO, 检查,
            out=true;
    ...
    pub fn debug(kind:&'static str,regs:Vec<&Reg>){
        ...
        match kind {
            ...
            "decolor"=>log_file!(color_spill_path,"decolor:{}({})",regs.get(0).unwrap(),regs.get(1).unwrap()),
