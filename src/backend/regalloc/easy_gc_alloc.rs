...
pub fn alloc(func: &crate::backend::func::Func) -> super::structs::FuncAllocStat {
    ...
    for to_color in to_colors {
        ...
        if !this_available.is_available(to_color.get_type()) {
            ...
            let nbs = intereference_graph.remove(&to_color).unwrap();
            for nb in nbs {
                intereference_graph.get_mut(&nb).unwrap().remove(&to_color);
            }
            availables.remove(&to_color);
            continue;
        }
