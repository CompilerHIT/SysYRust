...
impl  Allocator {
    pub fn new()->Allocator{
        Allocator { i_regs:Vec::new(),f_regs:Vec::new(), 
            ...
            regs_available:HashMap::new(), i_interference_graph: HashMap::new(),f_interference_graph:HashMap::new()
                ...
                if let Some(color)=color {
                    ...
                    if let Some(neighbors)=self.i_interference_graph.get(reg){
                        for neighbor in neighbors {
                            self.regs_available.get_mut(neighbor).unwrap().use_ireg(color);
                        }
                    }
                }else{
            ...
            for reg in self.f_regs.iter() {
                ...
                if let Some(color)=color {
                    ...
                    if let Some(neighbors)=self.f_interference_graph.get(reg){
                        for neighbor in neighbors {
                            self.regs_available.get_mut(neighbor).unwrap().use_freg(color);
                        }
                    }
                }else{
    ...
    fn simplify(&mut self)->bool{
        ...
        let mut out =true;
        let refresh=|reg:i32,colors:&HashMap<i32,i32>,regs_available:&mut HashMap<i32,RegUsedStat>,interference_graph:&HashMap<i32,HashSet<i32>>,kind:ScalarType| {
            ...
            if let Some(neighbors)=interference_graph.get(&reg) {
                ...
                for neighbor in neighbors {
                    ...
                    if let Some(color)=colors.get(neighbor){
                        if kind==ScalarType::Int { new_usestat.use_ireg(*color)}
                        else if kind==ScalarType::Float{new_usestat.use_freg(*color)}
                    }
                }
        ...
        let mut one_simplify=|target_reg:i32,colors:&mut HashMap<i32,i32>,interference_graph:&mut HashMap<i32,HashSet<i32>>,kind:ScalarType|->bool{
            // 遍历目标寄存器的周围寄存器
        ...
        if let Some(i_reg)=self.i_interence_reg {
            if one_simplify(i_reg,&mut self.icolors,&mut self.i_interference_graph,ScalarType::Int) {
        ...
        if let Some(f_reg  )=self.f_interence_reg {
            if one_simplify(f_reg,&mut self.fcolors,&mut self.f_interference_graph,ScalarType::Float) {
    ...
    fn spill(&mut self){
        ...
        let cost=|reg:i32,inference_graph:&HashMap<i32,HashSet<i32>>|{
            return self.costs_reg.get(&reg).unwrap()/inference_graph.get(&reg).unwrap_or(&HashSet::new()).len() as i32;
        };
