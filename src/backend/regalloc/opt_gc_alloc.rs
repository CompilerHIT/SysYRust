...
impl Regalloc for Allocator {
    fn alloc(&mut self, func: &crate::backend::instrs::Func) -> FuncAllocStat {
        ...
        for i in 0..max_times {
            let ok=regalloc::merge_alloc(func, &mut out.dstr, &mut out.spillings, 
            ...
            if !ok {
                log_file!("merge_times.txt","func:{},merge times:{}",func.label,i+1);
                break;
            }
            if i==max_times-1 {
                log_file!("merge_times.txt","unend!func:{},merge times:{}",func.label,i+1);
            }
        }
