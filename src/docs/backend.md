...
│      param    │
└───────────────┘
sp指向栈顶，s0代表栈底。来自caller函数的超过参数寄存器数量的参数在栈的最底部，紧邻着它的是callee saved寄存器；
...
取决于上层函数。callee saved是被调用函数需要保存的寄存器，取决于上层函数在发生call之后是否use，若未use则被调用函数不用保存；caller save是调用者函数发生函数调用时需要保存和恢复的寄存器，取决于该调用者函数自身在call是否use，若未use则不用保存。
在函数内联后，只存在main函数和递归函数，且递归函数的上层函数为自身或main，因此可以很容易获取到上层函数的信息。
为了保证最优效果，当函数内部的不同发生对同一个函数的调用，应该将这些相同函数当做不同函数处理，以保证最优的reg use信息